    ...
    <section className="bg-bunker-800 flex flex-col min-h-screen p-3">
      ...
      {movieDetails && (
        <>
          <div className="flex flex-col gap-20 items-center justify-center md:flex-row">
            <div
              className="absolute bg-center bg-cover blur-lg filter inset-0 z-0"
              style={{
            ...
            <div className="backdrop-blur-lg backdrop-filter bg-opacity-30 bg-white movie-poster p-4 relative rounded-2xl shadow-lg z-10">
              <div className="flex flex-col items-center justify-between">
                ...
                />
                <DetailsActionBar movie={movieDetails} />
              </div>
            </div>
            <div className="backdrop-blur-lg backdrop-filter bg-opacity-40 bg-white flex p-4 relative rounded-2xl shadow-lg w-[500px] z-10">
              <div className="flex flex-col items-center justify-between">
                <h1 className="font-bold text-3xl text-bunker-900">
                  {movieDetails.title}
                </h1>
                ...
                <div className="flex items-center space-x-2">
                  <span className="font-light italic text-bunker-900">
                ...
                <div className="flex flex-row gap-2 items-center justify-between p-3">
                  {movieDetails.genres.map((genre) => (
                    <span
                      key={genre.id}
                      className="backdrop-blur-lg backdrop-filter bg-opacity-10 bg-white p-2 rounded-2xl shadow-lg text-bunker-900"
                    >
                      {genre.name}
                    </span>
                  ))}
      ...
      {movieReviews && movieReviews.length > 0 && (
        <div className="flex flex-col items-start justify-start my-24 p-6 w-full">
          <h1 className="font-bold font-sans text-3xl text-gray-400">
            Reviews
          </h1>
          <div className="flex flex-col gap-4 p-3">
            {movieReviews.map((review) => (
              <div
                className="backdrop-blur-lg backdrop-filter bg-opacity-30 bg-white p-4 relative rounded-2xl shadow-lg z-10"
                key={review.id}
              >
                <div className="flex flex-col gap-2">
                  <div className="flex flex-row justify-between">
                    <div className="flex gap-1 items-center">
                      ...
                      <span className="text-west-side-200">
                        {review.author}
                      </span>
                    </div>
                    <div className="flex gap-1 items-center">
                      <span className="text-west-side-200">
                        {new Date(review.created_at).toLocaleString('EN')}
                      </span>
                    </div>
                  </div>
                  <p
                    className={`text-gray-200 ${
                      expandedReviewId === review.id
                        ? ''
                        : 'h-[200px] overflow-hidden text-clip'
                    }`}
                    dangerouslySetInnerHTML={{ __html: review.content }}
                  ></p>
                  <button
                    onClick={() => toggleReviewExpansion(review.id)}
                    className="text-east-bay-200"
                  >
